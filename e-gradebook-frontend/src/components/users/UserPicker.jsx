...
export default function UserPicker({ role, value = [], onChange }) {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      setErr(null);
      try {
        const res = await usersApi.list({ role, page: 1, limit: 200 });
  ...
  return (
    <div className='rounded-2xl bg-white p-4 shadow-inner'>
      ...
      {loading && <p className='px-1 py-2 text-sm text-gray-600'>Loading…</p>}
      {err && (
      ...
      {!loading && !err && (
        <ul className='max-h-64 overflow-auto space-y-2 pr-1'>
          {filtered.length === 0 ? (
            ...
            filtered.map((u) => {
              const selected = selectedSet.has(String(u.id ?? u._id));
