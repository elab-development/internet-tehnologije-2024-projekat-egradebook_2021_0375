...
export default function AssignChildrenModal({
  ...
  const onSubmit = async (e) => {
    ...
    setErr(null);
    try {
      await usersApi.setChildren(parent.id ?? parent._id, childIds);
      onSaved?.();
    } catch (e) {
      setErr(e.message || 'Failed to update children.');
    } finally {
      setSaving(false);
    }
  };
      ...
      <div className='w-full max-w-2xl rounded-2xl bg-white p-6 shadow-2xl'>
        <div className='mb-4 flex items-center justify-between'>
          <h3 className='text-lg font-semibold text-gray-900'>
            Set children for{' '}
            <span className='text-green-700'>{parent?.fullName}</span>
          </h3>
          <button
            onClick={onClose}
        ...
        <form onSubmit={onSubmit} className='space-y-6'>
          <UserPicker role='student' value={childIds} onChange={setChildIds} />
